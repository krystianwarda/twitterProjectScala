def FRA_clean_duplicates():
    ...
    for tempUser, tempStatusNumber in accountsToRecheckDict.items():
        print(tempUser)
        time.sleep(2)
        df_userTweets = pd.read_sql(f"SELECT * FROM FRA_tweets001 WHERE `user_screen_name` = '{tempUser}'", con=database_connection)
        print(len(df_userTweets.index))
        time.sleep(3)
        cursor.execute(f"DELETE FROM FRA_tweets001 WHERE `user_screen_name` = '{tempUser}';")
        df_userTweets = df_userTweets.drop_duplicates(subset=['id', 'created_at'])
        print(len(df_userTweets.index))
        df_userTweets.to_sql(con=database_connection, name='FRA_tweets001', if_exists='append', index=False)

    for tempUser, tempStatusNumber in accountsToRecheckDict.items():
        print(tempUser)
        time.sleep(2)
        df_userTweets = pd.read_sql(f"SELECT * FROM FRA_accounts_ledger WHERE `user_screen_name` = '{tempUser}'", con=database_connection)
        time.sleep(3)
        cursor.execute(f"DELETE FROM FRA_accounts_ledger WHERE `user_screen_name` = '{tempUser}';")
        print(df_userTweets)
        df_userTweets = df_userTweets.drop_duplicates(subset=['user_id', 'user_screen_name'], keep='last')
        print(df_userTweets)
        df_userTweets.to_sql(con=database_connection, name='FRA_accounts_ledger', if_exists='append', index=False)
