def update_user_ledger():
    ...
    df_ledger = pd.read_sql('SELECT user_id FROM accounts_ledger', con=database_connection)
    ...
    for i in range(0, len(mainAccounts), 100):
        if len(info) > 1000:
            ...
            df.to_sql(con=database_connection, name='users', if_exists='append', index=False)
    ...
    try:    
        cursor.execute('DELETE t1 FROM mydb.users t1 \
            JOIN mydb.users t2 \
            ON t1.id = t2.id \
            AND t2.trackDate > t1.trackDate;').fetchall()
    except:
        print('test1')
