def FRA_get_likes():
    ...
    df_users = pd.read_sql('SELECT id, screen_name, favourites_count FROM FRA_users', con=database_connection)
    testDict= dict(zip(df_users.screen_name, df_users.favourites_count))
    df_check = pd.read_sql('SELECT * FROM FRA_accounts_ledger_likes', con=database_connection)
    ...
    for tempName, tempStatusNumber in accountsToRecheckDict.items():
        ...
        try:
            ...
            df_update.to_sql(con=database_connection, name='FRA_accounts_ledger_likes', if_exists='append', index=False)
            ...
            df.to_sql(con=database_connection, name='FRA_likes001', if_exists='append', index=False)
    ...
    for tempUser, tempStatusNumber in accountsToRecheckDict.items():
        ...
        print(tempUser)
        df_userTweets = pd.read_sql(f"SELECT * FROM FRA_likes001 WHERE `user_screen_name` = '{tempUser}'", con=database_connection)
        print(len(df_userTweets.index))
        time.sleep(3)
        cursor.execute(f"DELETE FROM FRA_likes001 WHERE `user_screen_name` = '{tempUser}';")
        df_userTweets = df_userTweets.drop_duplicates(subset=['id', 'created_at', 'text'])
        print(len(df_userTweets.index))
        df_userTweets.to_sql(con=database_connection, name='FRA_likes001', if_exists='append', index=False)



    for tempUser, tempStatusNumber in accountsToRecheckDict.items():
        print(tempUser)
        df_userTweets = pd.read_sql(f"SELECT * FROM FRA_accounts_ledger_likes WHERE `user_screen_name` = '{tempUser}'", con=database_connection)
        cursor.execute(f"DELETE FROM FRA_accounts_ledger_likes WHERE `user_screen_name` = '{tempUser}';")
        print(df_userTweets)
        df_userTweets = df_userTweets.drop_duplicates(subset=['user_id', 'user_screen_name'], keep='last')
        print(df_userTweets)
        df_userTweets.to_sql(con=database_connection, name='FRA_accounts_ledger_likes', if_exists='append', index=False)