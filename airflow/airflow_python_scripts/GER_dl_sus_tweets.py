def GER_dl_sus_tweets():
    ...
    for tempName, tempStatusNumber in accountsToRecheckDict.items():
        ...
        try:
            ...
            for fid in Cursor(api.user_timeline, id=source__user_screen_name).items(tempStatusNumber):
                ids.append(fid)
            ...
            df_ledger.to_sql(con=database_connection, name='GER_accounts_ledger', if_exists='append', index=False)
            ...
            df.to_sql(con=database_connection, name='GER_tweets001', if_exists='append', index=False)
